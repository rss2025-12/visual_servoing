...
class ParkingController(Node):
    ...
    def __init__(self):
        ...
        # self.declare_parameter("distance_error_thresholds", [-0.6, 0.2]) # Boundaries for when the car is considered [too close, too far] from the cone for angle adjustment manuevers
        ...
        self.lookahead_distance = 1*(1.5*self.VELOCITY)**2
        self.L = 0.1
    ...
    def relative_cone_callback(self, msg):
        ...
        # self.get_logger().info(f'park {self.parking_distance}')
        target_dist = np.sqrt(self.relative_x**2+self.relative_y**2)
        # self.get_logger().info(f'targ {target_dist}')
        ...
        if not self.line_follower:
            if (dist_error < self.distance_error_threshold[1] and dist_error > self.distance_error_threshold[0]):
                steering_angle = 0.0
                velocity = 0.0
            # elif (dist_error > self.distance_error_threshold[1] and dist_error < self.distance_error_threshold[0]):
            #     steering_angle = 0.0
            #     velocity = 0.0
